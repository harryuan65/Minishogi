	...
	while (true) {
		...
		do {
			...
			do {
				...
				while (true) {
					...
					if (minishogi.IsGameOver()) {
						...
						fm_mg.SendMsg(indicator, 2 * sizeof(int), gameMode == 6);
						break;
					}
					...
					case Human:
						...
						fm_mg.SendMsg(indicator, sizeof(int), gameMode == 6);
						fm_mg.SendMsg(legalMoves, (endMove - legalMoves) * sizeof(Move), gameMode == 6);
						//fm.Send: Movelist
						if (gameMode != 6)
					...
					case AI:
						...
						indicator[0] = AIPV;//AI
						fm_mg.SendMsg(indicator, sizeof(indicator), gameMode == 6);
						fm_mg.SendMsg(pv, sizeof(pv), gameMode == 6);
			...
			do {
				...
				fm_gm.RecvMsg(buffer, sizeof(buffer), gameMode == 6);
			} while (gameMode == 6 && buffer[0] != RESTART);
		} while (gameMode == 6);
		cout << "---------- Total Report ----------\n";
