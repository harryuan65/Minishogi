...
#define SET_A_LIMIT_TO(X,MIN,MAX)  \
	...
	bool Weight::update(bool skip_update) {
		...
		if (!skip_update) {
			if (grad2[0] >= 0.1f)
				weight[0] = weight[0] - eta * grad[0] / sqrt(grad2[0]);

			if (grad2[1] >= 0.1f)
				weight[1] = weight[1] - eta * grad[1] / sqrt(grad2[1]);
		}
	...
	void AddGrad(Minishogi &m, Color rootTurn, double delta_grad) {
	...
	void Update(uint64_t epoch) {
		...
		auto func = [&](size_t id) {
			...
			for (int k1 = 9 * id; k1 < 9 * (id + 1); k1++) {
				for (Square k2 = SQUARE_ZERO; k2 < BOARD_NB; ++k2) {
					for (BonaPiece p = BONA_PIECE_ZERO; p < BONA_PIECE_INDEX_NB; ++p) {
					...
					for (Square k = Square(9 * id); k < (Square)(9 * (id + 1)); ++k) {
						for (BonaPiece p1 = BONA_PIECE_ZERO; p1 < BONA_PIECE_INDEX_NB; ++p1) {
							for (BonaPiece p2 = BONA_PIECE_ZERO; p2 < BONA_PIECE_INDEX_NB; ++p2) {
